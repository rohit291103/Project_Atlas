...
3. Fill out `docs/handoff/template.md`'s shape (create the template file itself, from the structure below, if it doesn't exist yet — this is the first time this skill has run in Atlas):
...
4. Save the filled template as `docs/handoff/<date>-<short-slug>.md` so it's retrievable later, and surface it to the user to copy into the other tool or hand to the other person — this skill doesn't call external AI APIs itself.
